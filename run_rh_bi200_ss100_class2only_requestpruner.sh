...
# run_rh_bi200_ss100_class1_requestpruner.sh (class-1-only version) and
# run_rh_bi200_ss100_class2only_pairpruner_v1final.sh (class-2-only Pair Pruner
...
BASE_OUT="new_tests/rh_class2only_requestpruner/bi200_ss100_seed${SEED}"

echo "=== [$(date +%H:%M:%S)] RH bi200_ss100 seed=${SEED} class2only requestpruner: baseline ==="
...
for THRESHOLD in 0.4 0.5 0.6; do
  echo "=== [$(date +%H:%M:%S)] RH bi200_ss100 seed=${SEED} class2only requestpruner: request t=${THRESHOLD} ==="
  for inst in $INSTANCES; do
...
echo "=== ALL RH bi200_ss100 seed=${SEED} class2only requestpruner RUNS DONE (4 variants x 6 instances) ==="
