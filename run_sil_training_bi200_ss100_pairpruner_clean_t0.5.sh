...
# outputs/models_v2/rgnn_mixed_c2_pw2_v1_final/), bi200/ss100, t=0.5.
# Same mixed-class SIL setup as run_sil_training_bi200_ss100_mixedclass_t0.5.sh
...
echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 pairpruner-clean t=0.5: baseline ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed 42 \
  --output_dir "outputs/sil_training_bi200_ss100_pairpruner_clean_baseline_t0.5" \
  > "sil_training_bi200_ss100_pairpruner_clean_baseline_t0.5.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: baseline (exit $?) ==="

echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 pairpruner-clean t=0.5: pair ==="
./venv/bin/python3 rtv_solver/main.py \
  --mode coaml \
  --input_dir "solutions/li_lim/manifests/" \
  --batch_interval 200 --step_size 100 --max_cardinality 2 \
  ...
  --seed 42 \
  --output_dir "outputs/sil_training_bi200_ss100_pairpruner_clean_pair_t0.5" \
  > "sil_training_bi200_ss100_pairpruner_clean_pair_t0.5.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: pair (exit $?) ==="

echo "=== ALL SIL bi200/ss100 pairpruner-clean t=0.5 RUNS DONE ==="
