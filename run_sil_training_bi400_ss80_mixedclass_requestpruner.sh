...
# Request Pruner sweep at bi400_ss80, thresholds 0.4/0.5/0.6, window-specific
# MLP checkpoint (see build_window_specific_request_pruners.py). Counterpart
# to run_sil_training_bi400_ss80_class1_requestpruner.sh (class-1-only version).
...
echo "=== [$(date +%H:%M:%S)] SIL bi400_ss80 seed=${SEED} mixedclass requestpruner: baseline ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed "$SEED" \
  --output_dir "outputs/sil_training_bi400_ss80_mixedclass_requestpruner_baseline_seed${SEED}" \
  > "sil_training_bi400_ss80_mixedclass_requestpruner_baseline_seed${SEED}.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: baseline (exit $?) ==="

for THRESHOLD in 0.4 0.5 0.6; do
  echo "=== [$(date +%H:%M:%S)] SIL bi400_ss80 seed=${SEED} mixedclass requestpruner: request t=${THRESHOLD} ==="
  ./venv/bin/python3 rtv_solver/main.py \
    --mode coaml \
    --input_dir "solutions/li_lim/manifests/" \
    --batch_interval 400 --step_size 80 --max_cardinality 2 \
    ...
    --seed "$SEED" \
    --output_dir "outputs/sil_training_bi400_ss80_mixedclass_requestpruner_request_t${THRESHOLD}_seed${SEED}" \
    > "sil_training_bi400_ss80_mixedclass_requestpruner_t${THRESHOLD}_seed${SEED}.log" 2>&1
  echo "=== [$(date +%H:%M:%S)] DONE: request t=${THRESHOLD} (exit $?) ==="
done

echo "=== ALL SIL bi400_ss80 seed=${SEED} mixedclass requestpruner RUNS DONE (4/4) ==="
