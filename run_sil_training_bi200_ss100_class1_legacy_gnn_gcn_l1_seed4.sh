...
# together before one shared linear layer), 1 message-passing layer(s), seed 4.
# Replaces the old (pre-fix) sum-combine seed-4/L1 run: ConvolutionalMeanLayer
...
echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 class1 legacy - GNN GCN 1L seed 4 ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed 4 \
  --output_dir "outputs/sil_training_bi200_ss100_class1_legacy_gnn_gcn_l1_seed4" \
  > "sil_training_bi200_ss100_class1_legacy_gnn_gcn_l1_seed4.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: class1 legacy GNN GCN 1L seed 4 (exit $?) ==="
