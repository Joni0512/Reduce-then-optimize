...
ROOT = Path(__file__).resolve().parents[2] / "outputs" / "outputs" / "eval_rh_no_learning_pruner"
OUT = Path(__file__).resolve().parents[2] / "outputs" / "eval_rh_no_learning_pruner_summary.csv"
...
for log_path in ROOT.glob("*/*/*/main.log"):
    ...
    run_name = f"{instance}_rh_no_learning_pruner"

    variant = "rh_no_learning_pruner"
    pruner = True
    threshold = 0.5
