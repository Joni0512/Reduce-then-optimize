"""
2026-09-10: TD-bootstrap critic target (gamma=0.99, tau=0.005) combined
with a GAT critic, on the FULL balanced 12-instance test set - see chat.
Corrected from the earlier 4-instance test_srl_td_bootstrap_gat.py: for a
clean comparison against the gcn+td_bootstrap 12-instance result
(run_srl_balanced_td_bootstrap_gamma099_tau0005_12instances.py, seed 42)
and the gat-no-bootstrap 12-instance result
(run_srl_balanced_gat_no_bootstrap_12instances.py), everything must run on
the same 12 instances. The 4-instance test found GAT did NOT collapse on
lrc207 under td_bootstrap (final SR 0.471, gcn collapsed to 0.000) -
checking whether that holds set-wide.
"""
...
def pretrain_shared_critic_gat() -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "run_srl_balanced_td_bootstrap_gat_12instances" / "critic_pretrain"
    ...
    for epoch in range(CRITIC_PRETRAIN_EPOCHS):
        ...
        print(f"[shared critic pretrain, gat, td_bootstrap] epoch {epoch} done")
...
if __name__ == "__main__":
    ...
    print("Shared GAT critic pretraining done - running all 12 test instances with td_bootstrap (gamma=0.99, tau=0.005).")

    failed = []
    for instance in TEST_INSTANCES:
        print(f"\n=== {instance} (balanced, gat, td_bootstrap gamma=0.99 tau=0.005) ===")
        try:
            ...
            train(
                ...
                label_suffix="_balanced_td_bootstrap_gat_gamma0.99_tau0.005",
    ...
    print(f"\n=== ALL 12 INSTANCES DONE (balanced, gat, td_bootstrap gamma=0.99 tau=0.005) - failed: {failed} ===")
