...
SIL experiment (bi400/ss200, t=0.4, 39 training files incl. 16 class-2
...
(t=0.5 version), just pointed at the t=0.4 run directories.
"""
import json
...
def find_run_dir(variant):
    base = Path(f"outputs/outputs/sil_training_bi400_ss200_mixedclass_{variant}_t0.4")
...
def make_bar_figure(data, ylabel, title, out_path, value_fmt="{:.1f}", ylim=None):
    ...
    fig.suptitle(f"SIL bi400/ss200, t=0.4 — Mixed-Class Training\n{title}", fontsize=12.5)
...
if __name__ == "__main__":
    ...
    make_bar_figure(service, "Service Rate (%)", "Service Rate (combined 12-instance val set)",
                     OUT_DIR / "fig_sil_mixedclass_bi400ss200_t04_service_rate.png", ylim=(0, 100))
    make_bar_figure(runtime, "Runtime (minutes, full 5-epoch training run)", "Runtime",
                     OUT_DIR / "fig_sil_mixedclass_bi400ss200_t04_runtime.png")
