"""
2026-09-05/06: first multi-seed GAT test (aggregator="gat" in CriticGNN) -
see chat and docs/SRL_Design.md's GAT plan section. Baseline hyperparameters
throughout (sigma=1.0, critic_lr=1e-3, actor_lr=1e-4, replay_capacity=40 -
all train() defaults, unchanged), so the only difference vs. the known gcn
baseline is the aggregator. Deliberately WITHOUT the route-clique edge
extension (critic_use_route_clique=False, still the existing chain-only
Request-Request edges) - companion script
replicate_gat_routeclique_12instances_seeds.py tests WITH the clique
extension for comparison. 5 seeds, all 12 balanced test instances.
"""
...
USE_ROUTE_CLIQUE = False


def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    critic = CriticGNN(aggregator="gat")
    critic_optimizer = torch.optim.Adam(critic.parameters(), lr=1e-3)
    output_dir = REPO_ROOT / "outputs" / "replicate_gat_nocliqueue_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
    ...
    for epoch in range(CRITIC_PRETRAIN_EPOCHS):
        for instance in TRAIN_INSTANCES:
            ...
            pipeline = COAMLPipeline(
                config, cleared_payload, imitation_solution_path=input_path, critic=critic, critic_optimizer=critic_optimizer,
                critic_use_route_clique=USE_ROUTE_CLIQUE,
            )
...
if __name__ == "__main__":
    for seed in SEEDS:
        print(f"\n=== Pretraining GAT critic (no route-clique) for seed={seed} ===")
        shared_critic = pretrain_shared_critic_seeded(seed)

        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, aggregator=gat, no route-clique) ===")
            try:
                ...
                train(
                    ...
                    label_suffix=f"_balanced_gat_nocliqueue_seed{seed}",
    ...
    print("\n=== ALL SEEDS x 12 INSTANCES DONE (gat, no route-clique) ===")
