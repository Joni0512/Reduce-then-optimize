...
ROOT = Path(__file__).resolve().parents[2] / "outputs" / "outputs" / "eval_rh_no_learning_baseline"
OUT = Path(__file__).resolve().parents[2] / "outputs" / "eval_rh_no_learning_baseline_summary.csv"
...
for log_path in ROOT.glob("*/*/*/main.log"):
    ...
    run_name = f"{instance}_rh_no_learning_baseline"

    variant = "rh_no_learning_baseline"
    pruner = False
    threshold = None
