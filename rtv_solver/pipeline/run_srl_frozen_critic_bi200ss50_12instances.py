...
ACTOR_CHECKPOINT = str(list((REPO_ROOT / "outputs/outputs/sil_training_bi200_ss50_mixed_legacy_mlp_seed1").rglob("coaml_model_weights_best_val.pt"))[0])
...
if __name__ == "__main__":
    ...
    for instance in TEST_INSTANCES:
        print(f"\n=== {instance} (frozen shared critic, bi=200,ss=50) ===")
        try:
            train(
                instance=instance,
                episodes=EPISODES,
                ...
                label_suffix="_clipped_bi200_ss50",
            )
        except Exception as e:
            print(f"!!! {instance} FAILED: {e!r} - skipping, continuing with remaining instances.")
            failed.append(instance)

    print(f"\n=== ALL 12 INSTANCES DONE (bi=200,ss=50) - failed: {failed} ===")
