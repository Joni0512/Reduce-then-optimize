...
reward_mode="local_positive", WITHOUT the replay-buffer staleness fix
(use_stale_td_target=True - see coaml_pipeline.py's __init__ docstring),
for ONE seed passed via argv - see chat. Deliberately reproduces the
pre-fix bug (commit 1bf3a77) on purpose, as the "before" side of a clean
bufferfix vs. no-bufferfix comparison under local_positive - companion to
run_srl_balanced_td_bootstrap_gat_local_positive_bufferfix_single_seed.py
(identical config otherwise). Do NOT use use_stale_td_target=True for any
real training run - it's a comparison-only opt-in flag.

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.run_srl_balanced_td_bootstrap_gat_local_positive_nobufferfix_single_seed <seed>
"""
import sys
...
def pretrain_shared_critic_gat_seeded(seed: int) -> torch.nn.Module:
    # 2026-09-11: critic pretrain has no replay buffer involved at all
    # (train_critic=True, single-episode-averaged loss) - the staleness bug
    # only exists in train()'s replay-buffer path below, so pretrain is
    # identical to the bufferfix companion script.
    critic = CriticGNN(aggregator="gat")
    critic_optimizer = torch.optim.Adam(critic.parameters(), lr=1e-3)
    output_dir = REPO_ROOT / "outputs" / "run_srl_balanced_td_bootstrap_gat_local_positive_nobufferfix_12instances" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    seed = int(sys.argv[1])
    print(f"\n=== Pretraining shared GAT critic for seed={seed} (local_positive, NO bufferfix) ===")
    shared_critic = pretrain_shared_critic_gat_seeded(seed)

    for instance in TEST_INSTANCES:
        print(f"=== seed={seed} instance={instance} (balanced, gat, local_positive, td_bootstrap gamma=0.99 tau=0.005, NO bufferfix) ===")
        try:
            ...
            train(
                ...
                label_suffix=f"_balanced_td_bootstrap_gat_local_positive_nobufferfix_gamma0.99_tau0.005_seed{seed}",
                ...
                use_stale_td_target=True,
            )
        except Exception as e:
            print(f"!!! seed={seed} instance={instance} FAILED: {e!r} - skipping, continuing.")

    print(f"\n=== SEED {seed} x 12 INSTANCES (gat, local_positive, NO bufferfix) DONE ===")
