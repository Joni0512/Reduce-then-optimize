...
OUT_LONG = Path(__file__).resolve().parents[2] / "outputs" / "threshold_sweep_long.csv"
OUT_WIDE = Path(__file__).resolve().parents[2] / "outputs" / "threshold_sweep_wide.csv"

RUN_SOURCES = [
    ("coaml", "eval_coaml_pw5_t0.2", True, 0.2),
    ("coaml", "eval_coaml_pw5_t0.3", True, 0.3),
    ("coaml", "eval_coaml_pw5_t0.4", True, 0.4),
...
BASELINE_FILES = [
    ("coaml", Path(__file__).resolve().parents[2] / "outputs" / "eval_mixed_all_baseline_summary.csv"),
...
if df.empty:
    ...
    print()
    print("Mean summary:")
    print(df.groupby(["pipeline", "threshold"])[
