"""
2026-09-17/18: same as srl_behavior_cloning_vs_rho_loop.py (see its
docstring for the full method/design rationale), but with RHO at bi200/ss100
- the SAME horizon as the actor (was bi400/ss100) - see chat. Already
includes the exponential_prefix fix (unlike the very first same-horizon
attempt on 2026-09-16, which used the still-buggy legacy scoring rule and
crashed with ManifestConsistencyError). Point of this run: check whether,
now that the y*-construction bug is fixed, a same-horizon comparison still
shows the same drop in RHO's own quality (bi400/600's RHO service rate was
lower than bi200's, see chat's ILP_TIMEOUT hypothesis) and whether the
actor's ability to imitate RHO changes when there's no horizon gap left to
imitate across.
"""
...
ACTOR_STEP_SIZE = 100
# 2026-09-17: set to match the actor's own horizon (200), the "same horizon" variant (see chat/module docstring).
...
def run(output_dir: Path) -> BehaviorCloningVsRhoResult:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for instance in shuffled:
            try:
                ...
                print(f"[bc_vs_rho_loop_bi200] epoch {epoch}: SKIPPING {instance} - {e}")
                continue
        ...
        print(f"[bc_vs_rho_loop_bi200] epoch {epoch}/{EPOCHS}: mean_fy_loss={avg_loss}")
        ...
        if epoch % VAL_EVERY_N_EPOCHS == 0 or epoch == EPOCHS:
            ...
            print(f"[bc_vs_rho_loop_bi200] epoch {epoch}: val={val_rate:.4f} overfit_check={overfit_rate:.4f}")
    ...
    print(f"[bc_vs_rho_loop_bi200] mean RHO service rate over {len(TRAIN_INSTANCES)} train instances = {mean_train_rho_rate:.4f}")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Behavior Cloning vs. live RHO (bi200 actor vs. bi200 RHO) - 38-instance split")
    ...
    print(f"[bc_vs_rho_loop_bi200] best val service rate = {best_val_service_rate:.4f} at epoch {best_epoch} -> {best_checkpoint_path}")
...
if __name__ == "__main__":
    _output_dir = REPO_ROOT / "outputs" / "srl_behavior_cloning_vs_rho_loop_bi200" / "run1_exppref"
    ...
    try:
        ...
        print(f"!!! srl_behavior_cloning_vs_rho_loop_bi200 CRASHED - full traceback written to {crash_path}")
        raise
