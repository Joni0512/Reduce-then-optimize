"""
2026-09-06: SECOND batch of seeds (47-51) for the GAT-nocliqueue test - see
chat. Runs in parallel with the original seeds-42-46 job (which is close to
its 24h serial_std time limit) on serial_long (48h) instead, to get more
seeds without waiting for the first job to finish. Same baseline
hyperparameters, same setup as replicate_gat_nocliqueue_12instances_seeds.py
otherwise - see that file's docstring for the full rationale.
"""
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_gat_nocliqueue_12instances_seeds2" / f"seed{seed}" / "critic_pretrain"
