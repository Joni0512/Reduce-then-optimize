"""
2026-09-09: TD-bootstrap critic target combined with a GAT critic
(aggregator="gat" in CriticGNN, 4-head attention) - see chat. Not tested
together before now - all TD-bootstrap runs so far used the gcn critic.
gamma=0.99, tau=0.005 (the original TD-bootstrap config), bi200/ss100.

Same 4 diagnostic instances as test_srl_td_bootstrap_bi400_ss200.py:
lrc207, lr210, lrc108 (collapsed under td_bootstrap+gcn at bi200/ss100,
seed 42), plus lc108 as the stable control.
"""
...
def pretrain_shared_critic_gat() -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "test_srl_td_bootstrap_gat" / "critic_pretrain"
    ...
    for epoch in range(CRITIC_PRETRAIN_EPOCHS):
        ...
        print(f"[shared critic pretrain, gat] epoch {epoch} done")
...
if __name__ == "__main__":
    ...
    print("Shared GAT critic pretraining done - running td_bootstrap (gamma=0.99, tau=0.005) on the diagnostic instances.")

    for instance in TEST_INSTANCES:
        print(f"\n=== {instance} (td_bootstrap, gat, gamma=0.99, tau=0.005) ===")
        try:
            ...
            train(
                ...
                label_suffix="_td_bootstrap_gat_gamma0.99_tau0.005",
    ...
    print("\n=== ALL GAT + TD-BOOTSTRAP TESTS DONE ===")
