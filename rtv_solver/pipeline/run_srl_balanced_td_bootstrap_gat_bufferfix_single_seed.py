"""
2026-09-10: same as run_srl_balanced_td_bootstrap_gat_bufferfix_12instances
.py, but for ONE seed passed via argv - see chat. Lets seeds 43-46 run as
separate parallel cluster jobs instead of one sequential job looping over
all of them (the earlier gcn+td_bootstrap seed sweep took ~5-6h per seed
sequentially - this lets 4 seeds finish in the time of one).

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.run_srl_balanced_td_bootstrap_gat_bufferfix_single_seed <seed>
"""
import sys
...
def pretrain_shared_critic_gat_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "run_srl_balanced_td_bootstrap_gat_bufferfix_12instances" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    seed = int(sys.argv[1])
    print(f"\n=== Pretraining shared GAT critic for seed={seed} ===")
    shared_critic = pretrain_shared_critic_gat_seeded(seed)

    for instance in TEST_INSTANCES:
        print(f"=== seed={seed} instance={instance} (balanced, gat, td_bootstrap gamma=0.99 tau=0.005, bufferfix) ===")
        try:
            ...
            train(
                ...
                label_suffix=f"_balanced_td_bootstrap_gat_bufferfix_gamma0.99_tau0.005_seed{seed}",
    ...
    print(f"\n=== SEED {seed} x 12 INSTANCES (gat, bufferfix) DONE ===")
