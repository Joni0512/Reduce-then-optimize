"""
2026-08-10: 5-seed (42, 1, 2, 3, 4) mean +/- std Service Rate and Runtime
bar charts and LaTeX table for the class-2-only RHO WITHOUT learning
(--mode offline, myopic rolling-horizon, no SIL/ML training) Pair Pruner
(GNN v1_final) threshold sweep, bi200/ss100, bi400/ss80, bi400/ss200.
Averages the single-seed results already computed per-seed in
outputs/new_tests/rh_class2only_pairpruner_v1final/{cfg}_seed{seed}/... (see
plot_rh_class2only_pairpruner_v1final_sweep.py for the single-seed version and
run_rh_*_class1_pairpruner_v1final.sh <seed> for how each seed's run was
produced). Error bars show +/-1 std across seeds.
"""
import json
import statistics
...
VAL_6 = ["lc207", "lc208", "lr210", "lr211", "lrc207", "lrc208"]
...
def instance_run_dir(cfg, seed, variant, inst):
    base = Path(f"outputs/new_tests/rh_class2only_pairpruner_v1final/{cfg}_seed{seed}/{variant}/{inst}")
...
def make_bar_figure(means, stds, cfg, ylabel, title, out_stem, value_fmt="{:.1f}", ylim=None):
    ...
    fig.suptitle(
        f"RHO without learning, {CONFIG_LABELS[cfg]}, class-2-only, mean over 5 seeds ({','.join(map(str, SEEDS))})\n"
        f"Pair Pruner (v1_final) — {title}",
        fontsize=11,
    )
...
def make_tex_figure_wrapper(stem, cfg, metric_desc):
    ...
    tex = (
        ...
        f"    \\caption{{{metric_desc} (mean $\\pm$ 1 std over 5 seeds: {seeds_str}) of the class-2-only Pair Pruner (GNN v1\\_final) threshold sweep under RHO without learning (myopic rolling horizon, no SIL training), {CONFIG_LABELS[cfg]}.}}\n"
        f"    \\label{{{label}}}\n"
        "\\end{figure}\n"
    )
...
def make_latex_table(results, out_path):
    ...
    lines += [
        r"\end{tabular}",
        rf"\caption{{Class-1-only Pair Pruner (GNN v1\_final) threshold sweep under RHO without learning (myopic rolling horizon, no SIL training): service rate (6-instance set) and total wall-clock runtime, mean $\pm$ 1 std over 5 seeds ({seeds_str}).}}",
        r"\label{tab:rh_class2only_pairpruner_v1final_multiseed_sweep}",
        r"\end{table}",
    ]
    out_path.write_text("\n".join(lines) + "\n")
...
if __name__ == "__main__":
    ...
    for cfg in CONFIGS:
        ...
        for variant in VARIANTS:
            ...
            print(f"{variant:10s}  service_rate={sr_m:5.1f}% (+/-{sr_s:.1f})   runtime={rt_m:6.2f} min (+/-{rt_s:.2f})   raw_sr={['%.1f'%x for x in srs]}")

        sr_stem = f"rh_class2only_pairpruner_v1final_{cfg}_service_rate_5seed"
        rt_stem = f"rh_class2only_pairpruner_v1final_{cfg}_runtime_5seed"
    ...
    make_latex_table(results, OUT_DIR / "rh_class2only_pairpruner_v1final_multiseed_sweep_table.tex")
