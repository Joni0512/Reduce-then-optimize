"""
2026-07-21: simple Service Rate + Runtime comparison for the mixed-class
SIL experiment (bi200/ss100, t=0.5, 39 training files incl. 16 class-2
instances, lc204 excluded). baseline/request/pair ran on the LRZ cluster,
both ran locally after the cluster job hit the 8h TIMEOUT mid-"both". No
orig-6/new-6 breakdown - combined 12-instance validation set only. Same
pattern as plot_sil_mixedclass_service_runtime.py (bi400/ss200 version).
"""
import json
...
def find_run_dir(variant):
    base = Path(f"outputs/outputs/sil_training_bi200_ss100_mixedclass_{variant}_t0.5")
...
def make_bar_figure(data, ylabel, title, out_path, value_fmt="{:.1f}", ylim=None):
    ...
    fig.suptitle(f"SIL bi200/ss100, t=0.5 — Mixed-Class Training\n{title}", fontsize=12.5)
...
if __name__ == "__main__":
    ...
    make_bar_figure(service, "Service Rate (%)", "Service Rate (combined 12-instance val set)",
                     OUT_DIR / "fig_sil_mixedclass_bi200ss100_service_rate.png", ylim=(0, 100))
    make_bar_figure(runtime, "Runtime (minutes, full 5-epoch training run)", "Runtime",
                     OUT_DIR / "fig_sil_mixedclass_bi200ss100_runtime.png")
