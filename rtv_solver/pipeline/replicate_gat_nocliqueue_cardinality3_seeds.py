...
combined with aggregator="gat" (no route-clique) - see chat, companion
script to replicate_cardinality3_12instances_seeds.py (same cardinality
change, plain gcn) for comparison. Same baseline hyperparameters otherwise
(sigma=1.0, critic_lr=1e-3, actor_lr=1e-4, replay_capacity=40). 5 seeds,
all 12 balanced test instances.
"""
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_gat_nocliqueue_cardinality3_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    for seed in SEEDS:
        print(f"\n=== Pretraining GAT critic (no route-clique, cardinality={MAX_CARDINALITY}) for seed={seed} ===")
        shared_critic = pretrain_shared_critic_seeded(seed)

        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, aggregator=gat, no route-clique, cardinality={MAX_CARDINALITY}) ===")
            try:
                ...
                train(
                    ...
                    label_suffix=f"_balanced_gat_nocliqueue_cardinality3_seed{seed}",
    ...
    print("\n=== ALL SEEDS x 12 INSTANCES DONE (gat, no route-clique, cardinality=3) ===")
