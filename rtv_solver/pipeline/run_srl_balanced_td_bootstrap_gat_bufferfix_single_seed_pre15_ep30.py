"""
2026-09-12: same as run_srl_balanced_td_bootstrap_gat_bufferfix_single_seed
.py, but with CRITIC_PRETRAIN_EPOCHS=15 (was 10) and EPISODES=30 (was 20) -
see chat. Checks whether more pretraining + more normal training improves
the GAT+TD-bootstrap+bufferfix result for seed=42, which already has a
10-pretrain/20-normal baseline to compare against. Separate output dir
(run_srl_balanced_td_bootstrap_gat_bufferfix_12instances_pre15_ep30) so the
existing seed42 10/20 results are not overwritten.

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.run_srl_balanced_td_bootstrap_gat_bufferfix_single_seed_pre15_ep30 <seed>
"""
import sys
...
from rtv_solver.pipeline.run_srl_balanced_frozen_12instances import (
    ACTOR_CHECKPOINT, TRAIN_INSTANCES, TEST_INSTANCES, BATCH_INTERVAL, STEP_SIZE,
)
...
def pretrain_shared_critic_gat_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "run_srl_balanced_td_bootstrap_gat_bufferfix_12instances_pre15_ep30" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    seed = int(sys.argv[1])
    print(f"\n=== Pretraining shared GAT critic for seed={seed} (pre15_ep30) ===")
    shared_critic = pretrain_shared_critic_gat_seeded(seed)

    for instance in TEST_INSTANCES:
        print(f"=== seed={seed} instance={instance} (balanced, gat, td_bootstrap gamma=0.99 tau=0.005, bufferfix, pre15_ep30) ===")
        try:
            ...
            train(
                ...
                label_suffix=f"_balanced_td_bootstrap_gat_bufferfix_gamma0.99_tau0.005_seed{seed}_pre15_ep30",
    ...
    print(f"\n=== SEED {seed} x 12 INSTANCES (gat, bufferfix, pre15_ep30) DONE ===")
