"""
2026-09-07: 5-seed gcn Replay-Buffer-alone baseline - see chat. The existing
"0.580 final" baseline number was a SINGLE seed (42) result; this replicates
it across 5 seeds (42-46, matching the first GAT seed batch) for a properly
matched multi-seed comparison against the GAT results
(replicate_gat_nocliqueue_12instances_seeds.py, 0.658 mean over 5 seeds).
Same baseline hyperparameters throughout (sigma=1.0, critic_lr=1e-3,
actor_lr=1e-4, replay_capacity=40, max_cardinality=2 - all train() defaults,
unchanged), plain gcn aggregator (CriticGNN default). 5 seeds, all 12
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_gcn_baseline_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    for seed in SEEDS:
        print(f"\n=== Pretraining shared critic (gcn baseline) for seed={seed} ===")
        shared_critic = pretrain_shared_critic_seeded(seed)

        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, gcn baseline) ===")
            try:
                ...
                train(
                    ...
                    label_suffix=f"_balanced_gcn_baseline_seed{seed}",
    ...
    print("\n=== ALL SEEDS x 12 INSTANCES DONE (gcn baseline) ===")
