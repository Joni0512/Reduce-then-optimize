"""
2026-09-11: same config as
run_srl_balanced_td_bootstrap_gat_local_positive_nobufferfix_single_seed.py
(use_stale_td_target=True, deliberately reproduces the pre-fix replay-buffer
staleness bug from commit 1bf3a77), but loops over all 5 seeds (43-47)
sequentially in ONE job instead of 5 separate parallel jobs - see chat.
Companion to run_srl_balanced_td_bootstrap_gat_local_positive_bufferfix_5seeds
.py for a clean before/after comparison under local_positive.
"""
import copy
import torch
...
if __name__ == "__main__":
    ...
    print(f"\n=== ALL {len(SEEDS)} SEEDS DONE (gat, local_positive, NO bufferfix) ===")
