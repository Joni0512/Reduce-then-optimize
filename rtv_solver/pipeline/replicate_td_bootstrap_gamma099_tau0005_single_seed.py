"""
2026-09-10: same TD-bootstrap (gamma=0.99, tau=0.005) 12-instance run as
replicate_td_bootstrap_gamma099_tau0005_12instances_seeds.py, but for ONE
seed passed via argv - see chat. Lets multiple seeds run as separate
parallel cluster jobs instead of one sequential job looping over all of
them (5482637 covers seeds 43-46 sequentially, ~5-6h per seed; this script
lets the remaining seeds run at the same time instead of waiting in line).

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.replicate_td_bootstrap_gamma099_tau0005_single_seed <seed>
"""
import sys
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_td_bootstrap_gamma099_tau0005_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    ...
    for instance in TEST_INSTANCES:
        print(f"=== seed={seed} instance={instance} (balanced, td_bootstrap gamma=0.99 tau=0.005) ===")
        try:
            ...
            train(
                ...
                label_suffix=f"_balanced_td_bootstrap_gamma0.99_tau0.005_seed{seed}",
