...
divergence diagnostic across 4 additional seeds (43-46) - see chat.
Extends both the single-instance (lrc207, 5 seeds) and the 12-instance
(seed=42 only) divergence findings together: does the pattern hold across
BOTH instances AND seeds simultaneously?

critic_tc_corr/critic_tc_mean_abs_diff are logged automatically by
coaml_pipeline.py/train_srl_single_instance.py whenever target_critic is a
distinct object and the replay buffer is active - no extra wiring needed.
"""
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_targetcritic_polyak03_diagnostic_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    for seed in NEW_SEEDS:
        ...
        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, target_critic Polyak tau=0.3, divergence diagnostic) ===")
            try:
                import copy
                ...
                train(
                    ...
                    label_suffix=f"_balanced_targetcritic_polyak0.3_diagnostic_all12_seed{seed}",
