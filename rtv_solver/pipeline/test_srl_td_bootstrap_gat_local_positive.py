"""
2026-09-10: TD-bootstrap (gamma=0.99, tau=0.005) with BOTH changes combined
- GAT critic AND reward_mode="local_positive" - see chat. Individually:
GAT alone fixed the collapse on lrc207/lr210 (final SR 0.471 both, vs. 0.0
with gcn), local_positive alone helped lrc108 (0.019 -> 0.442) but did NOT
fix lrc207/lr210 (still 0.0 with gcn). Testing whether combining both
closes that remaining gap, or whether GAT alone already captures the full
effect and local_positive adds nothing on top of it.

Same seed (42), replay buffer (capacity=40) as all other TD-bootstrap
tests, on the same 4 diagnostic instances (lrc207, lr210, lrc108, lc108).
"""
...
def pretrain_shared_critic_gat_local_positive() -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "test_srl_td_bootstrap_gat_local_positive" / "critic_pretrain"
    ...
    for epoch in range(CRITIC_PRETRAIN_EPOCHS):
        ...
        print(f"[shared critic pretrain, gat, local_positive] epoch {epoch} done")

    return critic


if __name__ == "__main__":
    print("Pretraining shared GAT critic (reward_mode=local_positive)...")
    shared_critic = pretrain_shared_critic_gat_local_positive()
    print("Shared GAT critic pretraining done - running td_bootstrap (gamma=0.99, tau=0.005, reward_mode=local_positive) on the diagnostic instances.")

    for instance in TEST_INSTANCES:
        print(f"\n=== {instance} (td_bootstrap, gat, reward_mode=local_positive, gamma=0.99, tau=0.005) ===")
        try:
            ...
            train(
                ...
                label_suffix="_td_bootstrap_gat_local_positive_gamma0.99_tau0.005",
    ...
    print("\n=== ALL GAT + local_positive TD-BOOTSTRAP TESTS DONE ===")
