"""
2026-07-20: figures for the mixed-class-training experiment (bi400/ss200,
t=0.5) - the corrected follow-up to plot_sil_extval_comparison.py. That
earlier experiment only extended VALIDATION_FILES (SIL trained class-1-only,
evaluated on both classes), confounding pruner-caused damage on class-2 with
plain domain shift. This run additionally extends TRAINING_FILES with the
pruners' own 16 class-2 train instances (17 minus lc204, excluded as a
160-200min/epoch outlier - see run_sil_training_bi400_ss200_mixedclass_t0.5.sh),
so SIL has actually learned class-2 patterns before being evaluated on them.
"""
import json
...
VARIANT_LABELS = {"baseline": "SIL (mixed-class, no pruner)", "request": "Request Pruner",
                   "pair": "Pair Pruner", "both": "Both"}
OUT_DIR = Path("outputs/pruner_comparison_summary")


def find_run_dir(variant):
    base = Path(f"outputs/outputs/sil_training_bi400_ss200_mixedclass_{variant}_t0.5")
...
if __name__ == "__main__":
    ...
    fig.suptitle("SIL bi400/ss200, t=0.5 — Mixed-Class Training: Does Damage Generalize to Class 2?\n"
                 "SIL now trained on both classes (39 files) - confound-free vs. earlier extval-only test",
                 fontsize=12.5)
    ...
    out_path = OUT_DIR / "fig_sil_mixedclass_comparison.png"
