...
- bi800/ss100 instead of bi400/ss100 - actor stays at bi200/ss100 (see
...
ACTOR_STEP_SIZE = 100
# 2026-09-17: widened further from 600 to 800 (see chat/module docstring).
RHO_BATCH_INTERVAL = 800
RHO_STEP_SIZE = 100
SEED = 42
ACTOR_LR = 1e-4
EPOCHS = 20
...
def run(output_dir: Path) -> BehaviorCloningVsRhoResult:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for instance in shuffled:
            try:
                ...
                print(f"[bc_vs_rho_loop_bi800] epoch {epoch}: SKIPPING {instance} - {e}")
                continue
        ...
        print(f"[bc_vs_rho_loop_bi800] epoch {epoch}/{EPOCHS}: mean_fy_loss={avg_loss}")
        ...
        if epoch % VAL_EVERY_N_EPOCHS == 0 or epoch == EPOCHS:
            ...
            print(f"[bc_vs_rho_loop_bi800] epoch {epoch}: val={val_rate:.4f} overfit_check={overfit_rate:.4f}")
    ...
    print(f"[bc_vs_rho_loop_bi800] mean RHO service rate over {len(TRAIN_INSTANCES)} train instances = {mean_train_rho_rate:.4f}")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Behavior Cloning vs. live RHO (bi200 actor vs. bi800 RHO) - 38-instance split")
    ...
    print(f"[bc_vs_rho_loop_bi800] best val service rate = {best_val_service_rate:.4f} at epoch {best_epoch} -> {best_checkpoint_path}")
...
if __name__ == "__main__":
    _output_dir = REPO_ROOT / "outputs" / "srl_behavior_cloning_vs_rho_loop_bi800" / "run1_exppref"
    ...
    try:
        ...
        print(f"!!! srl_behavior_cloning_vs_rho_loop_bi800 CRASHED - full traceback written to {crash_path}")
        raise
