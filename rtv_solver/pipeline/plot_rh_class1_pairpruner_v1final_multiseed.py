...
outputs/new_tests/rh_class1_pairpruner_v1final/{cfg}_seed{seed}/... (see
plot_rh_class1_pairpruner_v1final_sweep.py for the single-seed version and
...
def make_tex_figure_wrapper(stem, cfg, metric_desc):
    ...
    tex = (
        ...
        f"    \\caption{{{metric_desc} (mean $\\pm$ 1 std over 5 seeds: {seeds_str}) of the class-1-only Pair Pruner (GNN v1\\_final) threshold sweep under RHO without learning (myopic rolling horizon, no SIL training), {CONFIG_LABELS[cfg]}.}}\n"
        f"    \\label{{{label}}}\n"
        "\\end{figure}\n"
    )
...
def make_latex_table(results, out_path):
    ...
    lines += [
        ...
        r"\label{tab:rh_class1_pairpruner_v1final_multiseed_sweep}",
        r"\end{table}",
    ]
    out_path.write_text("\n".join(lines) + "\n")
...
if __name__ == "__main__":
    ...
    for cfg in CONFIGS:
        ...
        sr_stem = f"rh_class1_pairpruner_v1final_{cfg}_service_rate_5seed"
        rt_stem = f"rh_class1_pairpruner_v1final_{cfg}_runtime_5seed"
    ...
    make_latex_table(results, OUT_DIR / "rh_class1_pairpruner_v1final_multiseed_sweep_table.tex")
