"""
2026-09-05/06: multi-seed GAT test WITH the route-clique edge extension
(critic_use_route_clique=True) - see chat and docs/SRL_Design.md's GAT plan
section. Companion script to replicate_gat_nocliqueue_12instances_seeds.py
(same baseline hyperparameters, same seeds, same 12 instances) - the ONLY
difference is the Request-Request neighborhood: all requests on the same
route connected pairwise (clique) instead of only consecutive ones (chain).
"""
...
USE_ROUTE_CLIQUE = True


def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    critic = CriticGNN(aggregator="gat")
    critic_optimizer = torch.optim.Adam(critic.parameters(), lr=1e-3)
    output_dir = REPO_ROOT / "outputs" / "replicate_gat_routeclique_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    for seed in SEEDS:
        print(f"\n=== Pretraining GAT critic (route-clique) for seed={seed} ===")
        shared_critic = pretrain_shared_critic_seeded(seed)

        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, aggregator=gat, route-clique) ===")
            try:
                ...
                train(
                    ...
                    label_suffix=f"_balanced_gat_routeclique_seed{seed}",
    ...
    print("\n=== ALL SEEDS x 12 INSTANCES DONE (gat, route-clique) ===")
