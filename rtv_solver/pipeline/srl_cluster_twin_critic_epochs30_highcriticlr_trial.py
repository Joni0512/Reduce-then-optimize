"""
Same config as srl_local_twin_critic_epochs30_trial.py (best result so far: best_epoch=25,
best_val=0.7085, test=0.7616), except CRITIC_LR raised to 0.005 (actor_lr unchanged) - testing
whether an even more aggressive critic_lr (critic learning even faster relative to the actor)
helps further, on the cluster with EPOCHS=30 kept the same as that best local run.

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.srl_cluster_twin_critic_epochs30_highcriticlr_trial <seed>
"""
import sys
import traceback
...
ACTOR_LR = 0.0031923396291543868  # unchanged from sandy-sweep-60
CRITIC_LR = 0.005  # raised from 0.0010833586558285635


def main(seed: int) -> None:
    run_id = f"twin_critic_epochs30_highcriticlr_seed{seed}"
    output_dir = REPO_ROOT / "outputs" / "srl_training_sweep" / run_id
    print(f"=== srl_cluster_twin_critic_epochs30_highcriticlr_trial {run_id}: reward_mode={REWARD_MODE} actor_lr={ACTOR_LR} critic_lr={CRITIC_LR} tau={TAU} gamma={GAMMA} epochs={EPOCHS} seed={seed} use_twin_critic=True ===")
    try:
        ...
        print(f"=== srl_cluster_twin_critic_epochs30_highcriticlr_trial {run_id} DONE: best_epoch={result.best_epoch} best_val_service_rate={result.best_val_service_rate:.4f} test_service_rate={test_service_rate:.4f} ===")
    except Exception as e:
        crash_path = output_dir / "crash_traceback.txt"
        output_dir.mkdir(parents=True, exist_ok=True)
        with open(crash_path, "w") as f:
            traceback.print_exc(file=f)
        print(f"!!! srl_cluster_twin_critic_epochs30_highcriticlr_trial {run_id} FAILED: {e!r} - full traceback written to {crash_path}")
