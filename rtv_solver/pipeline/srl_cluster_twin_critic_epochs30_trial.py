"""
Exact same config as the best local run so far (srl_local_twin_critic_epochs30_trial.py,
twin_critic_epochs30_1: best_epoch=25, best_val=0.7085, test=0.7616) - only SEED differs,
to check how much of that result is seed luck vs. a robust effect across seeds.

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.srl_cluster_twin_critic_epochs30_trial <seed>
"""
import sys
import traceback
...
def main(seed: int) -> None:
    run_id = f"twin_critic_epochs30_seed{seed}"
    output_dir = REPO_ROOT / "outputs" / "srl_training_sweep" / run_id
    print(f"=== srl_cluster_twin_critic_epochs30_trial {run_id}: reward_mode={REWARD_MODE} actor_lr={ACTOR_LR} critic_lr={CRITIC_LR} tau={TAU} gamma={GAMMA} epochs={EPOCHS} seed={seed} use_twin_critic=True ===")
    try:
        ...
        print(f"=== srl_cluster_twin_critic_epochs30_trial {run_id} DONE: best_epoch={result.best_epoch} best_val_service_rate={result.best_val_service_rate:.4f} test_service_rate={test_service_rate:.4f} ===")
    except Exception as e:
        crash_path = output_dir / "crash_traceback.txt"
        output_dir.mkdir(parents=True, exist_ok=True)
        with open(crash_path, "w") as f:
            traceback.print_exc(file=f)
        print(f"!!! srl_cluster_twin_critic_epochs30_trial {run_id} FAILED: {e!r} - full traceback written to {crash_path}")
