"""
2026-09-25: twin-critic test at tau=0.001 (the ORIGINAL buggy value, see chat) instead of the
corrected 0.005 - deliberately reproduced here for a clean comparison against sandy-sweep-60
(single-critic, tau=0.001, best_val=0.6911, the best trial found in the buggy sweep) and
against srl_local_twin_critic_trial.py/srl_local_single_critic_baseline_trial.py (both
tau=0.005). Same actor_lr/critic_lr/reward_mode as those, only tau and use_twin_critic differ
across this small family of comparison runs.

Usage: ./venv/bin/python3 -m rtv_solver.pipeline.srl_cluster_twin_critic_tau0001_trial <seed>
"""
import sys
import traceback
...
def main(seed: int) -> None:
    run_id = f"twin_critic_tau0001_seed{seed}"
    output_dir = REPO_ROOT / "outputs" / "srl_training_sweep" / run_id
    print(f"=== srl_cluster_twin_critic_tau0001_trial {run_id}: reward_mode={REWARD_MODE} actor_lr={ACTOR_LR} critic_lr={CRITIC_LR} tau={TAU} gamma={GAMMA} seed={seed} use_twin_critic=True ===")
    try:
        ...
        print(f"=== srl_cluster_twin_critic_tau0001_trial {run_id} DONE: best_epoch={result.best_epoch} best_val_service_rate={result.best_val_service_rate:.4f} test_service_rate={test_service_rate:.4f} ===")
    except Exception as e:
        crash_path = output_dir / "crash_traceback.txt"
        output_dir.mkdir(parents=True, exist_ok=True)
        with open(crash_path, "w") as f:
            traceback.print_exc(file=f)
        print(f"!!! srl_cluster_twin_critic_tau0001_trial {run_id} FAILED: {e!r} - full traceback written to {crash_path}")
