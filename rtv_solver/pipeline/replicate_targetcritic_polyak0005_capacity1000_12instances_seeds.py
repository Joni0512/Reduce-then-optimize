"""
2026-09-05: replicates the 12-instance target_critic Polyak tau=0.005 +
replay_capacity=1000 (combined) result across 4 additional seeds (43-46), on
top of the local seed=42 run - see chat. Same motivation/pattern as the other
seed-replication scripts: checks whether the combined variant's seed=42
result (mean final SR 0.408, bimodal - 5/12 instances collapsed to 0.0) is
robust or a seed=42 artifact.
"""
...
def pretrain_shared_critic_seeded(seed: int) -> torch.nn.Module:
    ...
    output_dir = REPO_ROOT / "outputs" / "replicate_targetcritic_polyak0005_capacity1000_12instances_seeds" / f"seed{seed}" / "critic_pretrain"
...
if __name__ == "__main__":
    for seed in NEW_SEEDS:
        ...
        for instance in TEST_INSTANCES:
            print(f"=== seed={seed} instance={instance} (balanced, target_critic Polyak tau=0.005 + capacity1000) ===")
            try:
                ...
                train(
                    ...
                    label_suffix=f"_balanced_targetcritic_polyak0.005_capacity1000_seed{seed}",
