"""Mean L2 HP screening heatmap - best val service rate across learning_rate x
...
values = np.array([
    [0.7138, 0.7516, 0.7453],
    [0.7421, 0.7704, 0.7516],
    [0.7453, 0.7358, 0.7390],
])
...
im = ax.imshow(values, cmap=cmap, vmin=0.71, vmax=0.775, aspect="auto")
...
ax.set_title(
    "Mean L2 HP Screening — Best Val Service Rate\n"
...
for i in range(values.shape[0]):
    for j in range(values.shape[1]):
        v = values[i, j]
        label_color = "#fcfcfb" if v > 0.75 else TEXT_PRIMARY
...
plt.tight_layout()
out_path = "figures_export/mean_l2_hp_screening_heatmap.png"
plt.savefig(out_path, dpi=200, facecolor=fig.get_facecolor())
print("saved:", out_path)
pdf_path = "figures_export/mean_l2_hp_screening_heatmap.pdf"
