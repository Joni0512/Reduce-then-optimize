...
# together before one shared linear layer), 2 message-passing layer(s), seed 3.
# Replaces the old (pre-fix) sum-combine seed-3/L2 run: ConvolutionalMeanLayer
...
echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 class1 legacy - GNN GCN 2L seed 3 ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed 3 \
  --output_dir "outputs/sil_training_bi200_ss100_class1_legacy_gnn_gcn_l2_seed3" \
  > "sil_training_bi200_ss100_class1_legacy_gnn_gcn_l2_seed3.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: class1 legacy GNN GCN 2L seed 3 (exit $?) ==="
