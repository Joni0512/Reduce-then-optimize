...
for CARDINALITY in 3 4; do
  for INSTANCE in "${INSTANCES[@]}"; do
    ...
    echo "===== Running $INSTANCE | RH/offline + pruner | mc=${CARDINALITY} ====="
    ...
    python3 -m rtv_solver.main \
      ...
      --batch_interval 400 \
      --step_size 100 \
      --output_dir "outputs/eval_rh_no_learning_pruner_mc${CARDINALITY}/${INSTANCE}"
  done
done

echo "DONE RH/offline pruner evaluation (mc=3,4)"
