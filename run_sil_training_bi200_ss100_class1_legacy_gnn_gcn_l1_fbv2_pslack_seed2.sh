...
# run_sil_training_bi200_ss100_class1_legacy_gnn_gcn_l1_fbv2_seed2.sh (same
# config, pre-pickup_slack code, 94-dim) to see if the feature helps.
set -eu

echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 class1 legacy - GNN GCN 1L fbv2+pslack seed2 ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed 2 \
  --output_dir "outputs/sil_training_bi200_ss100_class1_legacy_gnn_gcn_l1_fbv2_pslack_seed2" \
  > "sil_training_bi200_ss100_class1_legacy_gnn_gcn_l1_fbv2_pslack_seed2.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: GCN 1L fbv2+pslack seed2 (exit $?) ==="
