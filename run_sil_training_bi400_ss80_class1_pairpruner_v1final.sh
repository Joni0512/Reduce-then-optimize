...
# bi400_ss80, thresholds 0.4/0.5/0.6, to verify earlier results. Baseline
...
echo "=== [$(date +%H:%M:%S)] SIL bi400_ss80 class1 pairpruner v1final: baseline ==="
./venv/bin/python3 rtv_solver/main.py \
  ...
  --seed 42 \
  --output_dir "outputs/sil_training_bi400_ss80_class1_pairpruner_baseline" \
  > "sil_training_bi400_ss80_class1_pairpruner_baseline.log" 2>&1
echo "=== [$(date +%H:%M:%S)] DONE: baseline (exit $?) ==="

for THRESHOLD in 0.4 0.5 0.6; do
  echo "=== [$(date +%H:%M:%S)] SIL bi400_ss80 class1 pairpruner v1final: pair t=${THRESHOLD} ==="
  ./venv/bin/python3 rtv_solver/main.py \
    --mode coaml \
    --input_dir "solutions/li_lim/manifests/" \
    --batch_interval 400 --step_size 80 --max_cardinality 2 \
    ...
    --seed 42 \
    --output_dir "outputs/sil_training_bi400_ss80_class1_pairpruner_pair_t${THRESHOLD}" \
    > "sil_training_bi400_ss80_class1_pairpruner_pair_t${THRESHOLD}.log" 2>&1
  echo "=== [$(date +%H:%M:%S)] DONE: pair t=${THRESHOLD} (exit $?) ==="
done

echo "=== ALL SIL bi400_ss80 class1 pairpruner v1final RUNS DONE (4/4) ==="
