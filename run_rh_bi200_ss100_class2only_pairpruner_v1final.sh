...
# run_rh_bi200_ss100_class1_pairpruner_v1final.sh (class-1-only version). Pair
...
BASE_OUT="new_tests/rh_class2only_pairpruner_v1final/bi200_ss100_seed${SEED}"

echo "=== [$(date +%H:%M:%S)] RH bi200_ss100 seed=${SEED} class2only pairpruner v1final: baseline ==="
...
for THRESHOLD in 0.4 0.5 0.6; do
  echo "=== [$(date +%H:%M:%S)] RH bi200_ss100 seed=${SEED} class2only pairpruner v1final: pair t=${THRESHOLD} ==="
  for inst in $INSTANCES; do
...
echo "=== ALL RH bi200_ss100 seed=${SEED} class2only pairpruner v1final RUNS DONE (4 variants x 6 instances) ==="
