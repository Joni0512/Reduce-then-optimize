...
# screening run, 1 message-passing layer, seed 6. Extends the L1 seed set
# (2,3,4,5) to 5 seeds total (2,3,4,5,6), matching the constant-5-seeds
# target across all layer/aggregator combinations.
set -eu

echo "=== [$(date +%H:%M:%S)] SIL bi200/ss100 class1 legacy - GNN Concat 1L seed 6 ==="
...
echo "=== [$(date +%H:%M:%S)] DONE: class1 legacy GNN Concat 1L seed 6 (exit $?) ==="
