...
for inst in INSTANCES:
    p = latest(BASE / "rh_baseline" / inst)
    if p:
        rows.append({
            "pipeline": "rh", "class": cls(inst), "instance": inst,
...
metrics = ["service_rate", "total_time", "serviced", "total_requests"]
...
overall = df.groupby("threshold")[["service_rate", "total_time"]].mean().reset_index()
